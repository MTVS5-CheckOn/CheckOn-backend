...
| MB-32 | `MemberSession.notificationsEnabled` 를 채울 원본이 PR6 까지 없다 | ✅ **CONFIRMED · PR6/V41 이 닫았다**(2026-08-27) — `V41__member_question_notification.sql` 이 `member_notification_preferences` 를 만들고 `MemberSessionService` 가 부재 시 `checkon.member.notification.default-enabled` 로 대체한다. 계약의 `notificationsEnabled` 키는 이제 항상 존재한다. `MemberSessionView.notificationsEnabled` 필드 복원 | A | ✅ 2026-08-27 |

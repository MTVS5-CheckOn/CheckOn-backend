...
| MB-38 | `max_claims` 소진을 판정할 수 없다 | 🔴 **OPEN · V40(PR5) 에서 닫는다**(2026-08-26 판정) — MB-04 는 초대 코드를 `max_claims=1` 1회용으로 확정했는데 **그것을 강제할 수단이 없다.** `member_invitation_claims_member_self_select`(V38)가 `account_id = current_checkon_account_id()` 로 격리해 `COUNT(*) WHERE invitation_id = ?` 는 **아무리 해도 최대 1**(내 것)이고, `uq_member_invitation_claims_pair (invitation_id, account_id)` 는 *같은 계정*의 재청구만 막는다. 총 claim 수를 제한하는 DB 제약은 없다. → **다른 계정이 이미 쓴 코드도 두 번째 계정이 등록에 성공한다.** PR4 는 없는 판정을 있는 척하지 않고 그 분기(`409 INVITE_ALREADY_CLAIMED` 의 「다른 계정 소진」)를 **구현하지 않았다.** 🔴 **`revoked_at` 을 「사용됨」으로 겸용하지 마라** — 마이그레이션 없이 닫히지만, `revoked_at` 은 「강사가 취소했다」는 뜻이라 「쓰였다」를 겹쳐 쓰면 강사가 둘을 **영원히 구분할 수 없고 그 모호함이 데이터에 남는다.** 「없는 값을 지어내지 마라」의 사촌이다 — **있는 값에 다른 뜻을 얹지 마라.** 🔴 **V40 에 그대로 넣을 SQL (DROP 없이 — `DROP CONSTRAINT` 는 R6 금지 SQL 이다):** `CREATE UNIQUE INDEX uq_member_invitation_claims_single_use ON member_invitation_claims (invitation_id);` · `ALTER TABLE member_invitation_codes ADD CONSTRAINT ck_member_invitation_codes_single_use CHECK (max_claims = 1);` 기존 pair 제약은 **그대로 둔다**. 🔴 **RLS 로 못 보는 것을 유니크 제약이 대신 막는다** — 두 번째 계정의 청구가 23505 로 실패하므로 **읽지 않고도 막힌다.** PR4 가 `parent_student_relationships`(V33:96-98)에서 이미 쓰는 바로 그 수법이다 | A | 🔴 V40(PR5) |
...
- MB-35 ~ MB-38 은 PR4 에서 신규 부여했고 **2026-08-26 에 넷 다 판정했다** — MB-35 수용 · **MB-36 계약 위반분은 PR4 에서 수정**(teachers 키 제거) · MB-37 수용(설계 §9-1 대로) · **MB-38 은 V40(PR5)** 에서 닫는다.

...
| MB-31 | `member_student_activation` 의 학부모·강사 교차 조회 경로 | **OPEN** — 🔴 **PR3 착수 전 차단 요소.** 불변식 4번 때문에 학부모의 자녀 활성화 조회·해제와 강사의 담당 학생 활성화 조회를 정책으로 열지 못했다(관계 테이블이 RLS 라 참조 시 재귀). 🔴 정책이 없으면 **SELECT 가 0행**이다 — 애플리케이션이 미리 확인해도 RLS 는 그걸 모른다. 해법은 설계 §6-4-2 **범위 세션 변수**(관계를 각자 컨텍스트로 확인 → 확인된 student_id 를 `checkon.scope_student_id` 에 넣기 → 정책이 그 값만 허용). `member_student_activation` 은 member 소유라 예약표 규칙 1 밖이므로 **V39(PR3)에서 정책을 붙일 수 있다** | A | PR3 착수 전 |
...
- MB-31 은 PR2 에서 신규 부여했다(`member_student_activation` 의 교차 조회).

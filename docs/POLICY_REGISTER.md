...
- Kafka 실행 경계: 스케줄러와 교사 요청 API는 기존처럼 `OperationalDetectionRunService`를 호출하고 Kafka 요청 Outbox를 만든다. 같은 백엔드 애플리케이션의 HTTP Adapter Consumer가 requested topic을 소비해 AI `POST /v1/detect`를 호출한다. AI 서버는 Kafka를 직접 소비·발행하지 않는다. Adapter는 HTTP 결과를 completed/failed topic으로 변환하고 기존 Result Consumer가 최종 상태와 신호를 저장한다.
- 이벤트 계약: Backend 내부 `checkon.risk-detection.requested.v1` → HTTP Adapter, HTTP Adapter → Backend Result Consumer `checkon.risk-detection.completed.v1`/`checkon.risk-detection.failed.v1`를 사용한다. 모든 메시지는 `schema_version=1.0` envelope와 `event_id`, `correlation_id(=run_id)`, `causation_id`, `tenant_alias`, `run_id`, `attempt_id`, `request_id`, `idempotency_key`, `snapshot_hash`를 가진다. AI와의 외부 계약 정본은 AI OpenAPI `POST /v1/detect`, 백엔드 내부 Kafka 정본은 `docs/contracts/risk-detection-kafka.asyncapi.yaml`이다.
- HTTP 변환: Adapter는 `payload` 전체를 요청 body로 보내고 `tenant_alias`를 `X-Tenant-Id`, HTTP 시도마다 새 추적 ID를 `X-Request-Id`, envelope의 `idempotency_key`를 `Idempotency-Key`로 보낸다. HTTP 200은 completed, 400·409는 재시도하지 않는 failed로 변환한다. 네트워크·빈 응답·5xx는 Kafka 재시도 후 DLT에 보존하고 최종 failed 이벤트를 발행한다.
...
- Adapter 전달 보장: requested Consumer는 AI의 `Idempotency-Key` 계약으로 HTTP 중복 호출을 안전하게 만들고 Kafka send 확인 뒤에만 요청 메시지 처리를 완료한다. completed/failed 중복 이벤트는 Result Consumer의 현재 attempt 검사와 Inbox로 한 번만 반영한다.

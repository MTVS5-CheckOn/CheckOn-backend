...
- hash: `detection_evidence`는 `snapshot_hash` 대상이다. 누락과 빈 배열은 동일하고, 배열은 `(kind, student_ref, at, source_table, record_id)`, JSON key는 중첩을 포함한 사전순, 배열은 계약별 키의 오름차순, 비ASCII는 원문 UTF-8, 구분자 공백 없는 JSON으로 정규화한다. `students`·`learning_events`·`alert_context`의 빈 배열은 유지하고 빈 `detection_evidence`만 키를 생략한다. `learning_events`의 optional 값과 `alert_context.resolved_at`은 null 키를 유지한다. 해시 입력에는 부동소수를 넣지 않으며 `snapshot_hash` 자신·`classes`·`passage_ref`는 제외한다. UTC 표기와 소수 초 규칙은 미확정이며 AI 팀의 실요청형·규칙별 벡터를 받기 전까지 기존 고정 벡터 정답값은 변경하지 않는다.
